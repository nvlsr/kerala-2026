...
for (const c of ours) {
  ...
  console.log(
    `  ${flag}  rank=${c.rank}/${c.total}  seat=${c.seat.toString().padStart(3)}  ${c.constituency.padEnd(20)}  ${c.name.padEnd(30)}  votes=${c.votes.toLocaleString().padStart(8)}`
  )
}
...
if (below.length > 0) {
  console.log(
    "CPI(M) candidates who finished 3rd or lower (NOT in Wikipedia top-2 view):"
  )
  for (const c of below) {
    console.log(
      `  seat=${c.seat}  ${c.constituency}  ${c.name}  rank=${c.rank}/${c.total}  votes=${c.votes.toLocaleString()}`
    )
  }
}
...
console.log()
console.log(
  "=== Aluva (76) — Wiki labels Shelna Nishad as CPI(M); ours as ??? ==="
)
const aluva = hist.find((h) => h.constituencyNumber === 76)!
const e21 = aluva.elections.find((e) => e.year === 2021)!
for (const c of e21.candidates) {
  console.log(
    `  ${(c.alliance || "?").padEnd(6)}  ${c.party.padEnd(40)}  ${c.name}  votes=${c.votes.toLocaleString()}`
  )
}
