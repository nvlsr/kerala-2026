...
- `twitter-responses/data/acs/*.md` + `twitter-responses/data/_lookups/ac-table.tsv` — per-AC tweet-response dossiers (markdown with YAML frontmatter) + flat aggregate TSV. Built by `scripts/analysis/tweet-dossiers/build-all.ts`. Gitignored. Not consumed by the site — parallel analytical layer used by the Twitter-response workflow at `twitter-responses/`. Schema documented in `twitter-responses/data/README.md`.
...
**Candidate-name normalisation**: ECI publishes the same candidate's name differently across cycles (case, periods, honorifics, caste suffixes — e.g. `"RAJEEV CHANDRASEKHAR"` vs `"Rajeev Chandrasekhar"`, `"K. Sasi"` vs `"K SASI"`, `"Adv. T. M. Thomas Isaac"` vs `"T M Thomas Isaac"`). **Always use `scripts/_lib/names.ts:normalizeName()` for equality comparisons** — never compare raw name strings directly, or you'll fragment the same person across multiple keys. Pair with `data/candidate-aliases.json` for spelling-variant overrides (`Kappan` ↔ `Kappen`) the normaliser can't fix. The util also exports `extractCasteSuffix()` and `nameSimilarity()` for "suspected same person but didn't merge" audits; see `scripts/analysis/audit-candidate-names.ts` for the canonical reference implementation.
