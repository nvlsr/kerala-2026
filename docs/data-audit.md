...
Audit notes from a one-off pre-launch alliance-coverage check. The script has since been removed; alliance integrity is now guarded by `scripts/pipeline/validate-alliances.ts` and the test suite.
